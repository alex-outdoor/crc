        ...
        $http.get(url).success(function(data, status, headers, config) {
            ...
            setTimeout(function(){
              for (var i=0; i<respondents.length; i++){
                ...
                // Methodology
                var methods = ['olbb', 'online_focus_group', 'ethnographic_interview', 'web_tdi', 'idi', 'focus_group', 'tdi', 'taste_test'];
                if (methods.indexOf(respondents[i].methodologies[0].name) >= 0){
                  $("[name=radio_@][value=&]".replace('@',i).replace('&',respondents[i].methodologies[0].name)).prop("checked",true);
                }else{
            ...
            // Deliverables
            var existing_deliverables = ['english_to_english', 'french_to_english', 'content_analysis', 'topline_report', 'full_report', 'summary_report', 'discussion_guide_design', 'screener_design']
            ...
            if (deliverables.length > 0){
              ...
              for(var j=0; j<deliverables.length; j++){
                ...
                $("[name=@_duration]".replace('@',deliverables[j].name)).val(deliverables[j].duration);
                if(existing_deliverables.indexOf(deliverables[j].name) < 0){ // This is for other deliverable
